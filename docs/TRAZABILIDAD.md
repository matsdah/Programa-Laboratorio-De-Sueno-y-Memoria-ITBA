...
rompe si se toca este archivo**. Estos once se rompen hacia todos lados.

Diez de ellos no implementan ninguna funcionalidad numerada y por eso no tienen
fila arriba: `config.py`, `utils/errors.py`, `utils/validation.py`,
`tools/base.py`, `tools/registry.py`, `app.py`, `analysis/mne_bridge.py`,
`ui/menus.py`, `ui/theme.py` y `ui/preferences.py`. Los otros dos —`core/recording.py` y `utils/units.py`— **sí tienen
